...
Requirements: `mcp==2.0.0`, `aiohttp>=3.9.0`, `pydantic>=2.0.0`, `requests>=2.31.0`. Python 3.10+. SQLite is built into Python (no additional install).

MCP SDK v2 automatically serves both protocol eras from the same process: modern `2026-07-28` clients use `server/discover`, while older clients continue to use the backward-compatible `initialize` handshake.
...
```
service_catalog.json  ──►  sync_catalog.py  ──►  azure_services.db  ──►  azure_pricing_mcp.py
...
- **`azure_pricing_mcp.py`** — MCP SDK v2 stdio entry point. The SDK handles modern discovery, legacy initialization, concurrent requests, cancellation, and protocol errors.
